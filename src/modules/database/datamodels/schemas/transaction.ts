...
export class Transaction implements DataObject {
    ...
    @ApiProperty({ type: String, enum: TransactionObjects }) @Prop({ type: String, enum: TransactionObjects }) originObject?: TransactionObjects;
    //id del objeto donde se genero la transaccion puede ser usuario cliente, banca o consorcio
    @ApiProperty() @Prop({ required: true, type: mongoose.SchemaTypes.ObjectId }) destinationId?: ObjectId;
    @ApiProperty({ type: String, enum: TransactionObjects }) @Prop({ type: String, enum: TransactionObjects }) destinationObject?: TransactionObjects;
